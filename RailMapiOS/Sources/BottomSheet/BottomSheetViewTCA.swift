...
struct BottomSheetViewTCA: View {
        ...
        NavigationStack(path: $store.scope(state: \.path, action: \.path)) {
            contentWithBars
                ...
                .sheet(item: $store.scope(state: \.signIn, action: \.signIn)) { signInStore in
                    SignInViewTCA(store: signInStore)
                }
                .sheet(item: $store.scope(state: \.account, action: \.account)) { accountStore in
                    AccountViewTCA(store: accountStore)
                }
        } destination: { store in
            switch store.case {
            case .journeyDetail(let detailStore):
                JourneyDetailViewTCA(store: detailStore)
            case .stationPicker(let pickerStore):
                StationPickerViewTCA(store: pickerStore)
            case .datePicker(let dateStore):
                DatePickerViewTCA(store: dateStore)
            case .confirmation(let confirmStore):
                ConfirmationViewTCA(store: confirmStore)
            }
        }
